...
export default function TripActionSheet({ trip, onClose, onEdit, onDelete, onLeave }) {
  ...
  const tripName = trip.name || 'ทริปนี้';
    ...
    <div className={`modal-overlay active${isClosing ? ' closing' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}>
      <div className="modal-sheet trip-action-sheet">
        ...
        <div className="trip-action-list">
          ...
          {isOwner && (
            <button className="trip-action-item delete" disabled={deleting} onClick={() => setConfirmType('delete')}>
              <span className="action-icon">
                <svg width="20" height="20" viewBox="0 0 16 16" fill="none">
          ...
          {isMember && (
            <button className="trip-action-item delete" onClick={() => setConfirmType('leave')}>
              <span className="action-icon">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <path d="M19 12H9M19 12L16 9M19 12L16 15"/>
                </svg>
              </span>
              ออกจากทริป
            </button>
      ...
      {confirmType === 'delete' && (
        <ConfirmModal
          icon="delete"
          message={`ต้องการลบ "${tripName}" หรือไม่ ?`}
          confirmText="ลบทริป"
      ...
      {confirmType === 'leave' && (
        <ConfirmModal
          icon="leave"
          message={`ต้องการออกจาก "${tripName}" หรือไม่ ?`}
          confirmText="ออกจากทริป"
